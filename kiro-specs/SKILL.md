...
Kiro identity and response style: see `references/kiro-identity.md`.
...
1. **Extract Properties**
   - Derive testable properties from EARS acceptance criteria
   - Focus on universal invariants (not single examples)
2. **Validate Traceability**
   - Link each property to the originating requirement
   - Note any redundant properties and consolidate
3. **Design Output**
   - Add a **Correctness Properties** section in `design.md`
   - Mark property-based tests as optional by default
...
- **Design must include correctness reflection** - Extract properties from EARS and add Correctness Properties section

## Workflow Diagram

Design includes correctness property reflection derived from EARS.

```mermaid
stateDiagram-v2
  state "Design\n(+Correctness Properties)" as Design
...
- Progress to **design** (how to build it + correctness properties)
...
- [Kiro Identity](references/kiro-identity.md) - Response style and code philosophy guidelines
- [Workflow Diagrams](references/workflow-diagrams.md) - Visual workflow references
