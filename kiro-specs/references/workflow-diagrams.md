...
stateDiagram-v2
  ...
  Design : Write Design + Correctness Properties
  Tasks : Write Tasks
...
graph LR
    ...
    B --> C{Approved?}
    C -->|No| B
    C -->|Yes| D[Design + Correctness Properties]
    D --> E{Approved?}
    E -->|No| D
    E -->|Yes| F[Tasks]
    F --> G{Approved?}
...
graph TD
    ...
    B -->|Create Design| E[Start Design + Correctness Properties]
    B -->|Update Design| F[Edit Design + Correctness Properties]
...
```mermaid
graph TD
    A[requirements.md] -->|Informs| B[design.md (incl. Correctness Properties)]
    B -->|Guides| C[tasks.md]
    C -->|References| A
    C -->|Implements| B
